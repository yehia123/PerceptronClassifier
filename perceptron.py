        ...
        for iteration in range(self.max_iterations):
            ...
            for f in trainingData:
                ...
                for label in self.legalLabels:
                    scores[label] = self.weights[label] * f

                maxy = scores.argMax()

                if maxy != trainingLabel:
                    self.weights[trainingLabel] = self.weights[trainingLabel] + f
                    self.weights[maxy] = self.weights[maxy] - f
...
The best choice of iteration after tuning it would be 6 iterations since 6 & 10 have the same correct 56% , 54% where as iteration 3 when testing it drops below 50%
"""